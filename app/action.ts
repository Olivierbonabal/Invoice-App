...
export async function getInvoiceById(invoiceId: string) {
    try {
        ...
        if (!invoice) {
            throw new Error("Facture non trouvée, dans la BdD");
        }
        return invoice
    } catch (error) {
        console.error(error)
...
export async function updateInvoice(invoice: Invoice) {
    try {
        ...
        if (!existingInvoice) {
            throw new Error(`Facture avec Identifiant ${invoice.id} introuvable.`);
        }
...
export async function deleteInvoice(invoiceId: string) {
    try {
        ...
        if (!deleteInvoice) {
            throw new Error("Erreur Suppression de la facture.");
        }
    } catch (error) {
        console.error(error)
    }
